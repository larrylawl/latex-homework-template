...
\begin{homeworkProblem}
    (a) Prove that \( h \) is consistent with all +ve training examples observed thus far, not including \( d \).

    \begin{proof}
        Let \( P(n) = h_n \) is consistent with \( D_{n-1} \forall n \in \mathbb{N} \), 
        where \( h_n \) denote the h for the nth iteration and \( D_{n-1} =\left\{\left\langle x_{k}, c\left(x_{k}\right)\right\rangle : c(x_k) = 1 \right\}_{k=1, \ldots, n - 1} \)
        \\ Note that \( O(k) = n - 1 \neq n \) as the prove does not include the current training example \( d \).

        By mathematical induction,
        \begin{enumerate}
            \item \textbf{Base Case:} 
            \begin{enumerate}
                \item \( P(1) \implies D = \emptyset \)
                \item h is trivially consistent with \( D_0 \). (by Proposition 1)
                \item \( P(1) \)  is true.
            \end{enumerate}
            \item \textbf{Inductive Step:}
            \begin{enumerate}
                \item Assume \( P(k) \) is true. 
                \\ That is, \( h_k \) is consistent with \( D_{k-1} \)
                \item To prove \( P(k + 1) \) is also true. 
                \\ That is, \( h_{k+1} \) is consistent with \( D_k \)
                \item By the end of the kth iteration
                \begin{enumerate}
                    \item (\( h_k \) is consistent with \( D_{k-1} \)) \( \wedge \) (\( h_k \) is consistent with \( d_k \)) \( \implies \) \( h_k \) is consistent with \( D_k \)
                    \nolinebreak (by inductive hypothesis and algorithm)
                    \item \( S = \{h_k\} \cup \{s \in S: \text{s is consistent with} D_{k-1} \} \) (by algorithm)
                    \item \( S \) is consistent with \( D_{k}\)
                \end{enumerate}
                \item At the (k+1)th iteration, 
                \begin{enumerate}
                    \item \( h_{k+1} \geq_g s \) for each \( s \in S \) not consistent with \( D_{k+1}\)
                    \item \( h_{k+1} \) is consistent with \( D_k \) (by definition of \( \geq_g \))
                    \item \( P(k + 1) \) is true.
                \end{enumerate}
            \end{enumerate}
            \item We thus have that P(1) and \( \forall \mathbb{N}, P(k) \implies P(k + 1) \). So, by principle of mathematical induction, P(n) is true \( \forall \mathbb{N} \).
        \end{enumerate}
    \end{proof}
\end{homeworkProblem}
